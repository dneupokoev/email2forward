...
email2forward запускается по настроенному графику и подключается к указанному почтовому ящику, читает еще непрочитанные письма, парсит их и отправляет в указанные мессенджеры.
...
```
{"date":"2023-03-14","time":"09:00","send":"tmp","telegram":"id_группы","bitrix24":"id_группы","title":"Короткое сообщение ТЕСТ","only_with_word_in_pic":"warning","only_with_color_in_pic":"red"}

Примеры:
{"time":"09:00","send":"p","telegram":"id_группы"}
{"telegram":"id_группы","title":"Короткое сообщение ТЕСТ"}
{"telegram":"id_группы","only_with_word_in_pic":"warning"}
{"telegram":"id_группы","only_with_color_in_pic":"red"}
{"bitrix24":"id_группы","title":"Короткое сообщение ТЕСТ"}
{"send":"p","bitrix24":"id_группы","telegram":"id_группы"}
```
...
- send - что отправлять:
  - t = title (короткое сообщение из поля "title")
...
- bitrix24 - id группы, в которую должно быть отправлено
  - Как узнать идентификатор группы bitrix24:
    - в интерфейсе открыть группу -> в правом верхнем углу три точки -> копировать ссылку -> ссылка будет примерно такая: https://bitrix.domain.ru/online/?IM_DIALOG=chat99999 -> chat99999 - это идентификатор группы 
